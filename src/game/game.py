...
class Game:
    ...
    def draw_hearts(self) -> None:
        ...
        for i in range(self.lives):
            ...
            self.screen.blit(
                self.heart_sprites[self.current_heart_frame], (x_position, position_y))
    ...
    def remove_objects(self, objects_remove):
        self.objects = [
            obj for obj in self.objects if obj not in objects_remove]
        ...
        for obj in self.objects:
            if isinstance(obj, Bullet):
                max_x = (self.setup.screen_width / obj.meters_to_pixel) - \
                    (obj.size[0] / obj.meters_to_pixel)
                max_y = (self.setup.screen_height / obj.meters_to_pixel) - \
    ...
    def display_game_over(self):
        ...
        self.screen.fill((0, 0, 0))
        self.screen.blit(game_over_text, (self.screen.get_width() //
                         2 - game_over_text.get_width() // 2, 200))
        self.screen.blit(restart_text, (self.screen.get_width() //
    ...
    def pause_menu(self) -> bool:
        ...
        save_button_rect = self.pygame_engine.create_rect(
            self.screen.get_width() // 2 - 100, 300, 200, 80
        )
        continue_button_rect = self.pygame_engine.create_rect(
            self.screen.get_width() // 2 - 120, 450, 240, 80
        )

        while True:
            self.screen.fill((0, 0, 0))

            self.pygame_engine.draw_rect(self.screen, (0, 0, 255), Vector(save_button_rect.x, save_button_rect.y),
                                         (save_button_rect.width, save_button_rect.height), 1)
            self.pygame_engine.draw_rect(self.screen, (0, 255, 0), Vector(continue_button_rect.x, continue_button_rect.y),
                                         (continue_button_rect.width, continue_button_rect.height), 1)

            self.screen.blit(
                save_text, (save_button_rect.x + 50, save_button_rect.y + 20))
            self.screen.blit(
                continue_text, (continue_button_rect.x + 20, continue_button_rect.y + 20))
